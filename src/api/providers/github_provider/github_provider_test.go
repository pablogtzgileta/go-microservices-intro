...
import (
	"errors"
	"github.com/pablogtzgileta/go-microservices-intro/src/api/clients/restclient"
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123,"name": "golang-tutorial-post","full_name": "pablogtzgileta/golang-tutorial-post"}`)),
		},
	})
	...
	assert.EqualValues(t, "golang-tutorial-post", response.Name)
	assert.EqualValues(t, "pablogtzgileta/golang-tutorial-post", response.FullName)
}